  ...
  useEffect(() => {
    ...
    async function load(){
      try{
        ...
        const six = (top.length === 6 ? top : currentClubs.sort((a,b)=>a.name.localeCompare(b.name)).slice(0,6))
          ...
          <aside className="lg:col-span-1 card border-0 flex flex-col">
            <div className="px-4 py-4">
              <div className="flex items-center justify-between">
                ...
                <Link href="/news" className="md:hidden text-sm font-semibold" style={{ color: '#75bc7a' }}>see all →</Link>
              </div>
            </div>
