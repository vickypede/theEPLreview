      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
        {articles.map((article) => (
          <article key={article.id} className="bg-card rounded-lg shadow-md p-3 sm:p-3 md:p-3 lg:p-4 hover:shadow-lg transition-shadow border border-border">
            <h2 className="text-base sm:text-base md:text-base lg:text-lg font-semibold leading-tight text-foreground mb-1 sm:mb-1 md:mb-1 lg:mb-2 line-clamp-2">{article.title}</h2>
            ...
            <a href={article.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-foreground text-xs sm:text-xs md:text-xs lg:text-sm font-medium">Read more →</a>
          </article>
        ))}
      </div>
