        ...
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
            {articles.map((article) => (
              <article key={article.id} className="bg-card rounded-lg shadow-md p-3 sm:p-4 md:p-4 lg:p-5 hover:shadow-lg transition-shadow border border-border">
                <h2 className="text-base sm:text-lg md:text-lg lg:text-xl font-semibold text-foreground mb-1 sm:mb-2 md:mb-2 lg:mb-2 line-clamp-2">{article.title}</h2>
                <p className="text-muted-foreground text-xs sm:text-sm md:text-sm mb-2 sm:mb-3 md:mb-3 lg:mb-4">{article.sourceName || article.source || ''}</p>
                {article.publishedAt?.toDate && (
                  <p className="text-muted-foreground text-xs mb-2 sm:mb-3 md:mb-3 lg:mb-4">{new Date(article.publishedAt.toDate()).toLocaleString()}</p>
                )}
                <a href={article.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-foreground text-xs sm:text-sm md:text-sm font-medium">Read more →</a>
              </article>
            ))}
          </div>
